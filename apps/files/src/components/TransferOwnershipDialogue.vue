...
export default {
	...
	methods: {
		...
		submit() {
			...
			const url = generateOcsUrl('/apps/files/transferownership')

			axios.post(url, data)
				.then(resp => resp.data)
				.then(data => logger.info('Transfer ownership request sent', { data }))
				.catch(error => logger.error('Could not send ownership transfer request', { error }))
		}
	}
}
</script>
